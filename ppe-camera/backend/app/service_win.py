...
def _install(cls, root: str, port: str, host: str, extra: dict) -> None:
    exe = sys.executable
    # pythonservice.exe cannot import our package; run this module with the
    # venv interpreter instead so the service starts in the right environment.
    win32serviceutil.InstallService(
        ...
        exeName=exe,
        exeArgs=f'-m app.service_win run-{cls._svc_name_.lower()}',
    )
