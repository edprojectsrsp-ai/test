...
export default function AIChatPage() {
  ...
  return (
    <div className="relative flex h-screen flex-col overflow-hidden bg-[#09090b] text-white">
      ...
      {/* ── Input ── */}
      <div className="relative z-10 border-t border-zinc-800/80 bg-black/60 px-4 py-4 backdrop-blur-md md:px-10">
        <div className="mx-auto max-w-3xl">
          <div className="flex items-end gap-3 rounded-2xl border border-zinc-700 bg-zinc-900/80 px-4 py-3 transition-colors focus-within:border-cyan-500/60">
            <textarea
              ref={inputRef}
              rows={1}
              value={input}
              ...
              placeholder={convId ? "Ask anything about your projects… (Enter to send)" : "Connecting to AI service…"}
              className="max-h-40 flex-1 resize-none bg-transparent text-sm text-white placeholder-zinc-600 outline-none disabled:opacity-50"
              style={{ scrollbarWidth: "thin" }}
            />
            <button onClick={() => send()} disabled={busy || !input.trim() || !convId}
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-500 to-blue-600 shadow-lg transition-all hover:scale-105 disabled:opacity-40 disabled:hover:scale-100">
              {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
            </button>
          </div>
          <p className="mt-2 text-center text-[10px] text-zinc-600">
            Shift+Enter for new line · Multi-LLM routing: Groq → Gemini → Ollama fallback
          </p>
        </div>
      </div>
    </div>
