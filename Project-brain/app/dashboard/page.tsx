    ...
    <div className="flex h-screen overflow-hidden bg-zinc-950 text-white">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          {/* ── PROJECT EXECUTIVE SUMMARY (when scheme selected) ── */}
          <AnimatePresence>
            {selectedSchemeId && (
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>

                {physLoading ? (
                  <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-6 text-center text-xs text-zinc-500 animate-pulse">
                    Loading project data…
                  </div>
                ) : detail ? (
                  <>
                    {/* ── Project Identity header bar ── */}
                    <div className="rounded-t-xl border border-zinc-700 bg-[#0b3d91]/40 px-5 py-3 flex items-center justify-between">
                      <div>
                        <p className="text-[10px] text-blue-300 uppercase tracking-widest mb-0.5">Project Executive Summary</p>
                        <h2 className="text-base font-bold text-white leading-tight">{detail.scheme_name}</h2>
                        <div className="flex items-center gap-2 mt-1 text-[10px] text-blue-200">
                          {detail.wbs_element && <span className="flex items-center gap-1"><Hash size={10} />{detail.wbs_element}</span>}
                          {detail.amr_no && <span className="flex items-center gap-1"><MapPin size={10} />AMR: {detail.amr_no}</span>}
                          <span className="capitalize">{detail.scheme_type?.replace(/_/g, " ")}</span>
                        </div>
                      </div>
                      <span className={`rounded-full px-4 py-1.5 text-xs font-bold text-white ${STATUS_BG[detail.status_key] ?? "bg-zinc-600"}`}>
                        {detail.status_text}
                        {detail.delay_days > 0 && <span className="ml-1.5 font-normal">({detail.delay_days}d overrun)</span>}
                      </span>
                    </div>

                    {/* ── 3-col: Identity | Stage-1 | Stage-2 ── */}
                    <div className="grid grid-cols-3 border-x border-b border-zinc-700 divide-x divide-zinc-700">

                      {/* Col 1: Contract Identity */}
                      <div className="p-4">
                        <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-3 font-bold">Contract Identity</p>
                        <div className="space-y-1.5">
                          {[
                            { label: "Contractor", value: detail.contractor },
                            { label: "Contract No", value: detail.contract_no },
                            { label: "Contract Value", value: detail.contract_value_cr != null ? `₹${detail.contract_value_cr.toFixed(2)} Cr` : null },
                            { label: "Effective Date", value: fmtDate(detail.effective_date) },
                            { label: "Sched. Completion", value: fmtDate(detail.schedule_completion_date) },
                            { label: "Planned Start", value: fmtDate(detail.planned_start_date) },
                            { label: "Est. Cost", value: detail.estimated_cost_cr != null ? `₹${detail.estimated_cost_cr.toFixed(2)} Cr` : null },
                            { label: "Sanctioned Cost", value: detail.sanctioned_cost_cr != null ? `₹${detail.sanctioned_cost_cr.toFixed(2)} Cr` : null },
                          ].map(({ label, value }) => (
                            <div key={label} className="flex justify-between text-[10px] border-b border-zinc-800/50 pb-1 last:border-0">
                              <span className="text-zinc-500">{label}</span>
                              <span className="text-zinc-200 font-medium text-right max-w-[55%] truncate" title={value ?? ""}>{value || "—"}</span>
                            </div>
                          ))}
                        </div>
                      </div>
