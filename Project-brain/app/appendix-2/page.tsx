 ...
 *  - Multi-revision tracking (Initial / Rev-A / Rev-B / ...)
 *  - Grid-based activity editor with weightage validation
 *  - Approval workflow with board meeting reference
 *  - One-click sync to Plan Engine (seeds plan_activities)
 *  - Revision compare view
 *
 * Place at: front/app/appendix2/page.tsx
 */

import React, { useEffect, useMemo, useState } from "react";
import {
  Plus, Trash2, FileCheck2, Sparkles, GitBranch, Layers,
  Save, RefreshCw, BookTemplate, Lock, Unlock, AlertCircle,
...
export default function Appendix2Page() {
  ...
  const [packages, setPackages] = useState<PkgLite[]>([]);
  ...
  useEffect(() => {
    if (!selectedSchemeId) {
      setPackages([]); setSelectedPackageId(null);
      ...
      <div className="max-w-[1500px] mx-auto">
        ...
        <div className="mb-8 border-b border-zinc-800 pb-6">
          <div className="flex items-center gap-3 mb-2">
            <BookTemplate className="text-cyan-400" size={28} />
            <h1 className="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500">
              Appendix-2 Engine
            </h1>
            <span className="ml-2 px-2 py-0.5 bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded text-[10px] font-bold">
              SMART AUTO-FILL
            </span>
          </div>
          <p className="text-zinc-400 text-sm">
            Formal baseline activity document. Auto-fill from templates, track revisions, approve for board, sync to Plan Engine.
          </p>
        </div>
        ...
        {/* Empty states */}
        {!selectedSchemeId && (
          <EmptyState icon={<FileText size={48} />} title="Select a scheme to begin" body="Choose any of your 76 schemes from the dropdown above." />
        )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm overflow-auto py-8">
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 w-full max-w-3xl mx-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5">
          ...
          <Input label="Document No" value={fields.document_no} onChange={(v) => setFields({ ...fields, document_no: v })} placeholder="APX2/SCH/YYYY" />
          <Input label="FY Baseline" value={fields.fy_baseline} onChange={(v) => setFields({ ...fields, fy_baseline: v })} placeholder="FY26-27" />
          <Input label="Scheduled Start" type="date" value={fields.scheduled_start_date} onChange={(v) => setFields({ ...fields, scheduled_start_date: v })} />
          <Input label="Scheduled Finish" type="date" value={fields.scheduled_finish_date} onChange={(v) => setFields({ ...fields, scheduled_finish_date: v })} />
          <Input label="Total Scope Value (Cr)" type="number" value={fields.total_scope_value_cr} onChange={(v) => setFields({ ...fields, total_scope_value_cr: v })} />
          <Input label="Prepared By" value={fields.prepared_by_name} onChange={(v) => setFields({ ...fields, prepared_by_name: v })} placeholder="GM (Coke)" />
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 w-full max-w-md mx-4">
        ...
        <div className="space-y-3">
          <Input label="Approved By" value={fields.approved_by_name} onChange={(v) => setFields({ ...fields, approved_by_name: v })} placeholder="CMD SAIL / Director (Operations)" />
          <Input label="Approval Date" type="date" value={fields.approval_date} onChange={(v) => setFields({ ...fields, approval_date: v })} />
          <Input label="Board Meeting Reference" value={fields.board_meeting_ref} onChange={(v) => setFields({ ...fields, board_meeting_ref: v })} placeholder="PCSB Meeting 142/2024" />
        </div>
