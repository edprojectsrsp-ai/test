...
def my_packages(user_id: int, db: Session = Depends(get_db)):
    """Return packages assigned to the current user (PM or site_engineer access)."""
...
def activities_for_package(package_id: int, db: Session = Depends(get_db)):
    """Return active plan activities for entering daily progress."""
    rows = db.execute(text("""
...
async def post_diary_entry(
    ...
    row = db.execute(text("""
        ...
        ON CONFLICT (activity_id, actual_date) DO UPDATE SET
            ...
            photo_urls=ARRAY(SELECT unnest(daily_actuals.photo_urls) UNION SELECT unnest(EXCLUDED.photo_urls))
    ...
    """), {"aid": activity_id, "d": actual_date, "q": actual_qty, "area": area_of_work,
           "mp": manpower_count, "w": weather_conditions, "rem": remarks, "uid": user_id,
           "lat": location_lat, "lng": location_lng, "ph": photo_urls}).mappings().first()
    db.commit()
    out = dict(row)
    out['photo_urls'] = list(out['photo_urls'] or [])
    return DiaryEntryOut(**out)


@router.post("/diary/submit", response_model=DiaryEntryOut)
async def post_diary_entry_submit(
    activity_id: int = Form(...),
    actual_date: Optional[date] = Form(None),
    actual_qty: float = Form(...),
    area: Optional[str] = Form(None),
    manpower: Optional[int] = Form(None),
...
async def quick_observation(
    ...
    """), {"pid": package_id, "t": observation_type, "ti": title, "d": description,
           "s": severity or '', "ph": photo_urls, "lat": location_lat, "lng": location_lng,
