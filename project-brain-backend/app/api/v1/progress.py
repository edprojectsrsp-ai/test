...
Endpoints live under `/api/v1/progress/*` because:
 - main.py includes this router with prefix="/api/v1"
 - this router uses prefix="/progress"
"""
...
def upsert_monthly_progress(payload: MonthlyProgressIn, db: Session = Depends(get_db)):
    """Upsert a monthly progress point for a package and keep cumulative fields consistent."""
    ...
    prev = (
        db.execute(
            text(
                """
                SELECT
                  COALESCE(MAX(cumulative_planned_pct),0) AS pp,
    ...
    cum_p = float(prev["pp"]) + float(payload.planned_progress_pct)
    cum_a = float(prev["pa"]) + float(payload.actual_progress_pct)
    variance = float(payload.actual_progress_pct) - float(payload.planned_progress_pct)
        ...
        db.execute(
            text(
                ...
                INSERT INTO plant_progress_monthly
                  (package_id, month_date,
                   planned_progress_pct, actual_progress_pct,
                   cumulative_planned_pct, cumulative_actual_pct,
                   risk_level, notes)
                VALUES
                  (:pid, :m, :pp, :pa, :cp, :ca, (:rl)::risk_level_enum, :n)
                ...
                RETURNING
                  progress_id, package_id, month_date,
                  planned_progress_pct, actual_progress_pct,
                  cumulative_planned_pct, cumulative_actual_pct,
                  variance_pct,
            ...
            {
                "pid": payload.package_id,
                "m": m,
                "pp": payload.planned_progress_pct,
                "pa": payload.actual_progress_pct,
                "cp": cum_p,
                "ca": cum_a,
                "rl": risk,
                "n": payload.notes,
...
def get_s_curve(package_id: int, db: Session = Depends(get_db)):
    """Return S-curve data plus a light linear regression forecast to 100%."""
    pkg = (
        db.execute(
            text(
                """
    ...
    points = [
        SCurvePoint(
            ...
            cumulative_actual_pct=float(r["cumulative_actual_pct"])
            if r["cumulative_actual_pct"] is not None
            else None,
            is_forecast=False,
        )
        for r in rows
    ...
    today = date.today()
    today_planned = float(rows[-1]["cumulative_planned_pct"]) if rows else None
    today_actual = (
        float(rows[-1]["cumulative_actual_pct"]) if rows and rows[-1]["cumulative_actual_pct"] is not None else None
    )
    today_var = (today_actual - today_planned) if today_actual is not None and today_planned is not None else None
    ...
    if len(actual_rows) >= 3:
        ...
        if den > 0:
            ...
            if slope > 0:
                ...
                confidence = round(max(0.0, min(100.0, r_sq * 100.0)), 1)

                ahead_behind = "ahead" if today_var is not None and today_var > 0 else "behind"
                pct_str = f"{abs(today_var):.1f}" if today_var is not None else "0.0"
                explainer = (
                    f"Based on last {n} months of progress (slope={slope:.2f}%/day). "
                ...
                if rows:
                    ...
                    cur = rows[-1]["month_date"]
                    cur_cum = float(rows[-1]["cumulative_actual_pct"] or 0)
                    days_per_step = 30
                    while cur < forecast_date and cur_cum < 100:
                        cur = cur + timedelta(days=days_per_step)
                        cur_cum = min(100.0, slope * (cur.toordinal() - x0) + intercept)
                        points.append(
                            SCurvePoint(
                ...
                # Persist forecast snapshot (best-effort; table may not exist in some deployments).
                try:
                    db.execute(text("UPDATE forecast_snapshots SET is_current=FALSE WHERE package_id=:pid"), {"pid": package_id})
                    db.execute(
                        text(
                            """
                            INSERT INTO forecast_snapshots(
                              package_id, snapshot_date,
                            ...
                            VALUES (
                              :pid, :sd,
                              'linear_regression'::forecast_method_enum, :fd, :c,
                              :fp, :ia, :ip,
                              :n, :e, TRUE,
                              :mp::jsonb
                            )
...
def get_progress_heatmap(db: Session = Depends(get_db)):
    ...
    rows = db.execute(
        text(
            ...
            ORDER BY
              CASE latest_risk WHEN 'red' THEN 1 WHEN 'amber' THEN 2 WHEN 'green' THEN 3 ELSE 4 END,
