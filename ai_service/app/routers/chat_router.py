...
def list_providers():
    ...
    Returns:
        {
          "available": ["openai", "gemini", "groq", "ollama"],   # actually configured
          "default":   "openai",                                  # configured AI_DEFAULT_PROVIDER
          "all_known": ["openai", "gemini", "groq", "ollama"]    # full universe
        }
    """
    ...
    return {
        ...
        "all_known": sorted(VALID_PROVIDERS),
    }
...
async def chat(payload: ChatIn):
    ...
    Optional `provider` in the payload forces a specific provider. When None
    (or "auto"), task-aware routing decides.
    """
    result = await chat_once(
        ...
        forced_provider=_normalize_provider(payload.provider),
