...
# Default user-facing pick when no override is set.  Configurable via env so
# you can switch the team default without code changes.
...
class ProviderRouter:
    ...
    def get_default_provider(self) -> Optional[str]:
        """Return the configured default if reachable, else first available, else None."""
        if DEFAULT_PROVIDER in self.providers:
            return DEFAULT_PROVIDER
        # If the configured default isn't configured (no API key), fall back to
        # ANY configured provider so the UI's default selection still works.
    ...
    def _resolve_chain(
        ...
        """Build the ordered provider chain for a single request."""
        if forced_provider:
            forced_provider = forced_provider.lower()
            if forced_provider not in VALID_PROVIDERS:
                logger.warning(f"Ignoring unknown forced_provider={forced_provider}")
            elif strict_forced:
                return [forced_provider]
            else:
                # Forced first, then a tiny fallback so an API hiccup doesn't
                # completely lose the request.  Ollama is always last because
                # it runs locally and degrades gracefully.
                chain = [forced_provider]
                if "ollama" not in chain:
                    chain.append("ollama")
                return chain
        # Default: full task-aware chain
    ...
    async def call(
        ...
        With `forced_provider`, that provider is tried first. By default a
        single-step fallback (to ollama) follows for resilience; pass
        `strict_forced=True` to disable that fallback (useful for testing
        a specific provider in isolation).
        """
        ...
        for provider_name in chain:
            ...
            logger.info(f"Trying {provider_name} for task={task_type} "
                        f"(forced={forced_provider}, strict={strict_forced})")
            try:
